...
function jointSorts(list,parentId){
    ...
    $.each(list,function(index,value){
        ...
        html+='<tr style="background-color: rgb('+colorTo+','+colorTo+','+colorTo+');cursor: pointer'+hidden+'" class="sortTr'+parentId+'" onclick="showChild('+value.id+')"><td>'+value.id+'</td><td>'+value.name+'</td><td><img src="'+value.image+'" width="60px" height="60px"></td><td>'+value.child.length+'</td><td>'+value.level+'</td><td><a href="/adm/sort/'+value.id+'" class="infoColor">查看 </a><a href="/adm/sort/'+value.id+'/add/'+value.name+'" class="infoColor"> 添加 </a><a href="/adm/sort/'+value.id+'/edit" class="infoColor"> 修改 </a><a href="" class="infoColor" onclick="delSort('+value.id+')"> 删除</a></td>';
