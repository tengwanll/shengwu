 ...
 * </i>
 *
 * @method boolean pushMsgToSingleDevice($channel_id,$msg,$opts)
 *         单播, 推送一条消息到指定设备<br />
 *         <b>
 ...
 *         $opts 可选参数, 包含 msg_type, msg_expires, deploy_status几项可选内容; <br />
 *         
 * @method boolean pushToSmartTag($type,$msg,$opts)
 ...
 *         
 * @method boolean pushBatchUniMsg($channel_ids,$msg);
 ...
 *         $opts 可选参数, 包含
 * 
 * @method boolean queryMsgStatus($msg_id);
 *         查询消息发送状态
 *         <br/>
 *         $msg_id 在发送时, 由服务端返回的 msg_id
 *
 * @method boolean queryTimerRecords($timer_id,$opts);
 *         根扰timer_id查询一个定时或周期的消息发送记录
 *         <br/>
 *         $timer_id 在发送消息时,由服务端产生的timer_id
 *         $opts 可选参数, 包含 start, limit, start_time, end_time等几项可选内容;<br />
 *
 * @method boolean queryTopicRecords($topic_id,$opts);
 *         根扰topic查询一个topic下的消息发送记录 
 *         <br/>
 *         $topic_id 发送消息时指定的topic
 *         $opts 可选参数, 包含
 *         
 * @method boolean queryTimerList($opts);
 *         查询当前应用下的定时消息及周期消息.
 *         <br/>
 *         $opts 可选参数, 包含timer_id, start, limit等几项可选内容, 当指定timer_id时, 仅返回目标timer相关的信息.
 *         
 * @method boolean createTag($tag);
 *         创建一个tag
 *         <br/>
 *         $tag tag名称
 *         
 * @method boolean deleteTag($tag);
 *         删除一个tag 
 *         <br/>
 *         $tag tag名称
 *         
 * @method boolean queryTags($opts);
 *         查询当前应用下的tag列表 
 *         <br>
 *         $opts 可选参数, 包含tag, start, limit, 当指定tag时, 仅返回目标tag相关信息
 *         
 * @method boolean addDevicesToTag($tag,$channel_ids);
 *         向一个tag下添加设备 
 *         <br/>
 *         $tag tag名称 <br />
 *         $channel_ids 需要添加的一组channel_id
 *         
 * @method boolean delDevicesFromTag($tag,$channel_ids);
 *         从一个tag下删除一批设备 
 *         <br/>
 *         $tag; <br />
 *         $channel_ids 需要删除的一组channel_id
 *         
 * @method boolean queryDeviceNumInTag($tag);
 *         查询一个tag下的设备数量
 *         <br/>
 *         $tag tag名称
 *         
...
class PushSDK extends BaseSDK {
    ...
    function __construct($apiKey = null, $secretKey = null, $curlopts = array()) {
        ...
        if($apiKey !== null && $secretKey !== null){
            ...
            $this -> setApiKey(BaiduConfig::default_apiKey);
            $this -> setSecretKey(BaiduConfig::default_secretkey);
        }
