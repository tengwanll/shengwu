...
class BaseSDK {
    ...
    // 是否抛出异常
    private static $suppressException = true;
    ...
    protected function onError($requestId = null, $code = 0, $msg = '') {
        ...
        if ($code > 0 && $code < 100) {
            ...
            $err = new ClientException("[$code] $msg", $code);
        } else {
            if ($code >= 400 && $code < 600) {
                // 只将Http的错误状态认为是网络错误. 200及300对于当前restApi来说将不能被理解.
                $type = self::ERR_NET;
                $err = new \Exception("[$code] $msg", $code);
            } else {
                if ($code >= 10000) {
                    // 大于10000应为服务端返回的错误
    ...
    function __construct($curlOpts = array()) {
        ...
        //self::$suppressException = BaiduConfig::SUPPRESS_EXCPTION;
        self::$suppressException = BaiduConfig::SUPPRESS_EXCPTION;
        $this->log = new PushSimpleLog(BaiduConfig::LOG_OUTPUT, BaiduConfig::LOG_LEVEL);

        try {

            $this->http = new HttpRequest(BaiduConfig::HOST, $curlOpts, $this->log);
        ...
        $this->signExpires = intval(BaiduConfig::SIGN_EXPIRES);
    ...
    public function newApiQueryTpl() {
        ...
        if ($this->deviceType === null && BaiduConfig::default_devicetype !== null) {
            $this->setDeviceType(BaiduConfig::default_devicetype);
        }
            ...
            if ($status == 200) {
                if (array_key_exists('response_params', $result)) {
                    ...
                    $this->onError(
                        $result ['request_id'],
                        4,
                        "http status is ok, but REST returned invalid json struct."
                    );
                }
            } else {
            ...
            if ($status == 200) {
                ...
                if ($status >= 400) {
                    ...
                    $this->onError(
                        null,
                        -1,
                        "Can not process the http response. with status ($status, $statusMsg), content ($limitContent);"
                    );
                }
            }
        }
